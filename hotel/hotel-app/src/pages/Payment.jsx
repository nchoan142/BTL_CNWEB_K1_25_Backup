...
const Payment = () => {
  ...
  const handleConfirm = () => {
    if (paymentOption === 'now') {
      ...
      alert(`Thanh toán thành công qua ${method.toUpperCase()}! Mã đặt phòng: ${booking.id}`);
    } else {
      // Logic thanh toán sau
      bookingService.updateStatus(booking.id, 'Pay at Hotel', 'Cash', originalPrice);
      alert(`Đặt phòng thành công! Vui lòng thanh toán ${originalPrice}$ khi đến khách sạn.`);
    }
